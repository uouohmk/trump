...
export function makeSnapshot(chart, date=koreanDate()){
 ...
 topics.sky=[topics.sky[0],...fortuneCards(chart,signIndex,planets,date),topics.sky[1]];
 topics.saju=[...sajuInsight(chart),...topics.saju];
 if(signs.length>1){const note=`태양 별자리가 ${signs.join(' 또는 ')}인 경계예요. 오늘의 풀이는 ${signs[0]}을 기준으로 먼저 보여드려요.`;topics.flow.unshift({title:'별자리 경계에서 읽는 운세',symbol:'☉',text:note});topics.work.unshift({title:'별자리 경계에서 읽는 운세',symbol:'☉',text:note});topics.sky[0].text+=' 뒤의 운세는 '+signs[0]+' 기준으로 먼저 살펴볼게요.';}
 return {date,topics,planets,signIndex,signCandidates:signs.map(s=>SIGNS.indexOf(s)),version:3};
}
