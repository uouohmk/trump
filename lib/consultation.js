...
export function makeSnapshot(chart, date=koreanDate()){
 ...
 const planets=[['Sun','태양','☉'],['Moon','달','☽'],['Mercury','수성','☿'],['Venus','금성','♀'],['Mars','화성','♂']].map(([body,name,symbol])=>({name,symbol,longitude:body==='Sun'?SunPosition(moment).elon:Ecliptic(GeoVector(body,moment,true)).elon}));
 const [year,month,day]=chart.solar.split('-').map(Number);
 const moments=chart.utc?[new Date(chart.utc)]:[0,23].map(hour=>new Date(localToUTC({year,month,day,hour,minute:hour?59:0},chart.zone)));
 ...
 const topics={
  sky:[{title:signs.join(' 또는 '),symbol:'☉',text:signs.length>1?'태어난 시간을 몰라 두 별자리의 경계에 있어요.':`태어난 날의 태양은 ${signs[0]} 구간에 있어요.`,note:'열대황도 12궁 기준',kind:'sign'},
  ...
  saju:[{title:`나의 일주 · ${hangul(chart.pillars.day)}`,symbol:chart.pillars.day,text:ELEMENT_STORIES[chart.dayElement],note:`일간의 오행: ${ELEMENTS[chart.dayElement]}`},
        ...[['year','태어난 해'],['month','태어난 달'],['hour','태어난 시간']].map(([key,title])=>({title,symbol:chart.pillars[key]||'?',text:chart.pillars[key]?`${hangul(chart.pillars[key])}예요. ${key==='year'?'사주의 연주에 해당해요.':key==='month'?'절기를 기준으로 정한 월주예요.':'출생 시간으로 정한 시주예요.'}`:key==='hour'?'시간을 몰라도 다른 세 기둥은 살펴볼 수 있어요.':'절기의 경계에 있어 하나로 정하지 않았어요.'})),
        {title:'나를 읽는 네 기둥',symbol:'四',text:'사주는 출생 정보를 간지로 옮긴 전통적인 해석 틀이에요. 한 기둥만으로 성격이나 삶을 단정하지 않아요.'}],
  tarot:[{title:TAROT[tarotIndex][0],symbol:String(tarotIndex).padStart(2,'0'),text:TAROT[tarotIndex][1],note:'메이저 아르카나 · 정방향',kind:'tarot'},
         {title:'카드가 건네는 질문',symbol:'✧',text:'이 문장에서 지금 내 상황과 닿는 부분은 무엇인가요?',note:'매일 같은 기록에는 같은 카드가 나와요. 무작위 추첨이나 미래 예측은 아니에요.'}]
 };
 return {date,topics,version:1};
}
